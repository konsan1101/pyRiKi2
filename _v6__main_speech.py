...
if __name__ == '__main__':
    ...
    #api      free, google, watson, azure, aws, nict, winos, macos,
    #lang     ja, en, fr, kr...
    ...
    if (True):
        ...
        img = qRiKi.getIconImage(filename='_speech_start_', )
        if (not img is None):
            qGuide.init(panel='7', title='_speech_start_', image=img, alpha_channel=0.5, )
            qGuide.open()
            guide_disp = True
            guide_time = time.time()
        ...
        while (main_core.proc_r.qsize() != 0) and (control == ''):
            ...
            if (res_name.lower() == '_guide_'):
                if (guide_disp == True):
                    ...
                    img = qRiKi.getIconImage(filename='_speech_guide_', )
                    if (not img is None):
                        qGuide.init(panel='7', title='_speech_guide_', image=img, alpha_channel=0.5, )
                        qGuide.setMessage(txt=res_value, )
                        #qGuide.open()
                        guide_disp = True
                        guide_time = time.time()
    ...
    if (True):
        ...
        img = qRiKi.getIconImage(filename='_speech_stop_', )
        if (not img is None):
            qGuide.init(panel='7', title='_speech_stop_', image=img, alpha_channel=0.5, )
            qGuide.open()
            guide_disp = True
            guide_time = time.time()
