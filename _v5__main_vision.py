...
class main_vision:
    ...
    def main_proc(self, cn_r, cn_s, ):
        ...
        busy_status_txts = _v5__qRiKi.qBusy_status_txts_class()
...
if __name__ == '__main__':
    ...
    if (camDev_max == 'none'):
        ...
        img = qRiKi.getIconImage(filename='_vision_start_', )
        if (not img is None):
            qGuide.init(panel='3', title='_vision_start_', image=img, alpha_channel=0.5, )
            qGuide.open()
            guide_disp = True
            guide_time = time.time()
        ...
        while (main_core.proc_r.qsize() != 0) and (control == ''):
            ...
            if (res_name.lower() == '_guide_'):
                if (guide_disp == True):
                    ...
                    img = qRiKi.getIconImage(filename='_vision_guide_', )
                    if (not img is None):
                        qGuide.init(panel='3', title='_vision_guide_', image=img, alpha_channel=0.5, )
                        qGuide.setMessage(txt=res_value, )
                        #qGuide.open()
                        guide_disp = True
                        guide_time = time.time()
    ...
    if (True):
        ...
        img = qRiKi.getIconImage(filename='_vision_stop_', )
        if (not img is None):
            qGuide.init(panel='3', title='_vision_stop_', image=img, alpha_channel=0.5, )
            qGuide.open()
            guide_disp = True
            guide_time = time.time()
