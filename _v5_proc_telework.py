...
class proc_telework:
    ...
    def check_proc(self, telework_text, cn_s):
        ...
        # 環境取得
            ...
            while (self.face_img is None) and ((time.time() - checkTime) < 15.00):
                for cam_dev in cam:
                    ...
                    if (not img is None):
                        ...
                        _, face_imgs = qFFmpeg.detect_face(inp_image=img, )
                        if (len(face_imgs) > 0):
            ...
            # 表示
            #if (not self.desktop_img is None):
            #    cn_s.put(['_telework_guide_', self.desktop_img])
            #if (not self.face_img is None):
    ...
    if (len(sys.argv) < 2):
        ...
        while ((time.time() - chktime) < 150):
            ...
            if   (res_name == '_telework_guide_image_'):
                qGuide.init(panel='0-', title='telework', image=res_value, alpha_channel=0.5, )
                qGuide.open()
