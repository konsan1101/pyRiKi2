...
qGuide2=_v5__qGuide.qGuide_class()
    ...
    def main_proc(self, cn_r, cn_s, ):
        ...
        while (self.proc_step == '5'):
            ...
            if (telework_thread is None) and (telework_switch == 'on'):
                ...
                telework_thread  = _v5_proc_telework.proc_telework(
                                    name='telework', id='0',
                                    runMode=self.runMode,
                                    )
            ...
            if (not telework_thread is None) and (telework_switch != 'on'):
            ...
            if (inp_name.lower() == 'telework'):
                if (not telework_thread is None):
...
if __name__ == '__main__':
    ...
    if (True):
        ...
        img = qRiKi.getIconImage(filename='_desktop_start_', )
        if (not img is None):
            qGuide.init(panel='9', title='_desktop_start_', image=img, alpha_channel=0.5, )
            qGuide.open()
            guide_disp = True
            guide_time = time.time()
        ...
        while (main_core.proc_r.qsize() != 0) and (control == ''):
            ...
            if (res_name.lower() == '_guide_'):
                if (guide_disp == True):
                    ...
                    img = qRiKi.getIconImage(filename='_desktop_guide_', )
                    if (not img is None):
                        qGuide.init(panel='9', title='_desktop_guide_', image=img, alpha_channel=0.5, )
                        qGuide.setMessage(txt=res_value, )
                        #qGuide.open()
                        guide_disp = True
                        guide_time = time.time()
    ...
    if (True):
        ...
        img = qRiKi.getIconImage(filename='_desktop_stop_', )
        if (not img is None):
            qGuide.init(panel='9', title='_desktop_stop_', image=img, alpha_channel=0.5, )
            qGuide.open()
            guide_disp = True
            guide_time = time.time()
